...
public class Transition
  ...
  private GeneralPath transition1;
...
//   }
  ...
  private void constructTransition() {
    Rectangle2D.Double rectangle = new Rectangle2D.Double((componentWidth - TRANSITION_WIDTH) / 2, 0, TRANSITION_WIDTH, TRANSITION_HEIGHT);
    transition = new GeneralPath();
    transition.append(rectangle, false);

    Rectangle2D.Double rect2 = new Rectangle2D.Double(rectangle.getX()+3, 3, rectangle.getWidth()-6, rectangle.getHeight()-6);
    transition1 = new GeneralPath();
    transition1.append(rect2, false);
...
//    Fix-Me: #Continuous-connecting-constraint this needs more reasoning to connect between continuous/discreet
//    Fix-me: place/transition for now there is no constraint
//      boolean isAllowed = isContinuous() ? someArc.getSource().isContinuous() : !someArc.getSource().isContinuous();
      boolean isAllowed = true;
      if ((proximityTransition.contains((int) unZoomedX, (int) unZoomedY)
          || transition.contains((int) unZoomedX, (int) unZoomedY))
          && areNotSameType(someArc.getSource()) && isAllowed) {
...
//   }
   ...
   */
  public List<Place> getPlaceInList() {
...
//
//    return placeInList;
    return new MapperList<>(getArcInList(), arc -> (Place) arc.getSource());
  }

  /**
   * update and get places from connectTo arcs
   */
  public java.util.List<Place> getPlaceOutList() {
//    if (arcOutList.isEmpty()) {
//      placeOutList.clear();
//      getArcOutList().stream().forEach(arc -> {
//        arcOutList.add(arc);
//        placeOutList.add((Place) arc.getTarget());
//      });
//    }

//    return placeOutList;
    return new MapperList<>(getArcOutList(), arc -> (Place) arc.getTarget());
  }
...
//	   int[] tokens = new int[numOfPlaces];
    ...
    for (Place p : getPlaceInList()) {
      if (p.getToken().listToken.isEmpty()) {
        System.out.println(String.format("Transition.checkStatusAndFire: input place %s lacks of token.", p));
        return false;
      }
    }
    //check if all output places have room since simple place can only hold at most one token
    for (Place p : this.getPlaceOutList()) {
      if (!p.getDataType().getPow() && p.getToken().getTokenCount() > 0) {
        boolean biDirectional = getPlaceInList().stream().anyMatch(place -> place.getName().equals(p.getName()));
        if (!biDirectional) {
          System.out.println(String.format("Place %s is not bidirectional", p.getName()));
          return false;
        }
      }
    }
    ...
    //TODO:: Consider introducing concurrency or pipe-lining. You do not need to wait until all the combination are prepared.
...
//    }
    //check
    if (tokCombs.isEmpty()) {
      //This transition has no input variable; but may have post-condition.
      tokCombs.add(new SymbolTable());
    }
    ...
    while (iTokCombs.hasNext() && !status) {
      ...
      Sentence s = FormulaUtil.parseFormula(this, errorMsg);
  ...
  static class MapperList<T, E> extends AbstractList<E> {

    private final List<T> mOriginalList;
    private final Function<T, E> mMapper;

    public MapperList(final List<T> pOriginal, final Function<T, E> pMapper) {
      mOriginalList = pOriginal;
      mMapper = pMapper;
    }
    ...
    public E get(int index) {
      return mMapper.apply(mOriginalList.get(index));
    }
