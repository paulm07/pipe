...
public class DataLayer
  ...
  public String pnmlFileName = null;
  /**
  ...
  public ArrayList<Transition> continuous = null;
  ...
  private Map<String, DataType> mDataTypes = new HashMap<>();
  private Map<DataTypeAware, String> mUnresolvedDataTypes = new HashMap<>();
  ...
  private void initializeMatrices() {
    ...
    continuous = new ArrayList<Transition>();
  ...
  public void addPetriNetObject(PetriNetObject pnObject, final boolean pMakeUnique) {
    ...
    if (pnObject instanceof Arc) {
      ...
      if (transition.isContinuous()) {
        continuous.add(transition);
      } else {
        unknown.add(transition);
      }
    }
...
//    System.out.println("Random Fired Transition Backwards" + ((Transition)transitionsArray.get(randonTransition)).getId());
  ...
  private void initializeDataLayerFromDOMElement(final DataLayer pDataLayer, final Element pRootElement) {
      ...
      if (node instanceof Element) {
        ...
        if (converter == null) {
          if ("datatype".equals(name)) {
            DataType dataType = toDataType(element);
            if (dataType != null) {
              mDataTypes.put(dataType.getName(), dataType);
            }
          }
    ...
    List<String> usedDataTypes = mPlacesMap.values().stream().map(p -> p.getDataType().getName()).collect(Collectors.toList());
    Iterator<Map.Entry<String, DataType>> iterator = mDataTypes.entrySet().iterator();
    while (iterator.hasNext()) {
      Map.Entry entry = iterator.next();
      if (!usedDataTypes.contains(entry.getKey())) {
        iterator.remove();
      }
    }

    mUnresolvedDataTypes.forEach((k, v) -> k.setDataType(mDataTypes.get(v)));
    mUnresolvedDataTypes.clear();

    Map<String, NormalArc> inverseArcHolder = new HashMap<>();
    for (Arc arc : getArcs()) {
      if (arc instanceof NormalArc) {
        NormalArc normalArc = (NormalArc) arc;
        String source = arc.getSource().getId();
        String target = arc.getTarget().getId();
        String backwardKey = String.format("%s-%s", target, source);
        if (inverseArcHolder.containsKey(backwardKey)) {
          NormalArc inverseArc = inverseArcHolder.get(backwardKey);
          if (arc.getVar() != null && arc.getVar().equals(inverseArc.getVar())) {
            normalArc.setInverse(inverseArc, true);
          }
          inverseArcHolder.remove(backwardKey);
        }
        else {
          String forwardKey = String.format("%s-%s", source, target);
          inverseArcHolder.put(forwardKey, normalArc);
        }
      }
...
//   }
  ...
  private String getAgentName(final String pnmlName) {
    String[] parts = pnmlName.split("[^a-zA-Z0-9]+");
    StringBuilder sb = new StringBuilder(pnmlName.length());
  ...
  private void mergeDataLayer(final DataLayer pDataLayer) {
    String agentName = getAgentName(pDataLayer.pnmlName);

    for (DataType dataType : pDataLayer.getDataTypes()) {
      String name = dataType.getName();
      if (mDataTypes.containsKey(name)) {
        name = String.format("%s_%s", name, agentName);
        if (mDataTypes.containsKey(name)) {
//          TODO:: need proper implementation
    ...
    while (objectIterator.hasNext()) {
      PetriNetObject petriNetObject = objectIterator.next();
      petriNetObject.setContainerNet(agentName);
      addPetriNetObject(petriNetObject);
    }
  }
  ...
  public List<String> getPlaceNames() {
    return mPlacesMap.values().stream().map(p -> p.getName()).collect(Collectors.toList());
  }

  public void setPnmlName(final File pnmlFile) {
    if (pnmlFile.exists() && !pnmlFile.isDirectory()) {
      String name = pnmlFile.getAbsolutePath().substring(pnmlFile.getParent().length()+1);
      int index = name.lastIndexOf('.');
      if (index != -1) {
        name = name.substring(0, index);
      }
      this.pnmlName = name;
      this.pnmlFileName = pnmlFile.getAbsolutePath();
    }
  }

  public DataType getDataType(final String pDataTypeName) {
    return mDataTypes.get(pDataTypeName);
  }

  public void addUnresolvedDataType(final String pDataTypeName, final DataTypeAware pObject) {
    mUnresolvedDataTypes.put(pObject, pDataTypeName);
  }

  public DataType toDataType(final Element pElement) {
    String dataTypeName = pElement.getAttribute("name");
    String dataTypeNType = pElement.getAttribute("ntype");
    String dataTypeTypes = pElement.getAttribute("types");
    String dataTypeIsPow = pElement.getAttribute("pow");
    String dataTypeNumOfElement = pElement.getAttribute("elements-count");
    String dataTypeIsDef = pElement.getAttribute("def");
    ...
    if (StringUtils.isNotBlank(dataTypeName) &&
      ...
      dt.setTypes(types);
      dt.setPow(Boolean.valueOf(dataTypeIsPow));
      dt.setDef(Boolean.valueOf(dataTypeIsDef));
  ...
  public Collection<DataType> getDataTypes() {
    return mDataTypes.values();
  }

  public boolean addDataType(final DataType pDataType) {
    if (!mDataTypes.containsKey(pDataType.getName())) {
      mDataTypes.put(pDataType.getName(), pDataType);
      return true;
    }
